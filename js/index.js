...
function handle(tr) {
	...
	if (txt.indexOf("无打卡") != -1 && txt.indexOf("异常") != -1) {
		console.log(1);
		...
		if (contentTable.childElementCount != 2) {
			...
			cTable[0].firstChild.value = "fa68ab6c-78c7-4270-97ca-76f76550b2db";
			addOnChange(cTable[0].firstChild);

		}
		content[0].children[1].firstChild.click();
	}
	else if (txt.indexOf("异常") != -1) {
		debugger
		var trrs = td.children[0].children;
		var times = trrs[3].innerText;
		var end = times.split("~")[1].split(":");
		...
		var startMin = Number(start[1]);
		cTable[2].children[1].value = 9;
		addOnChange(cTable[2].children[1]);
		cTable[2].children[2].value = startMin;
		addOnChange(cTable[2].children[2]);
		//debugger
		cTable[5].firstChild.children[0].click();
		if (start[0][0] == '0') {
			//cTable[3].children[1].value = start[0][1];
		} else {
			//cTable[3].children[1].value = start[0];
		}
		//cTable[3].children[1].value = start[0];
		//addOnChange(cTable[3].children[1]);
		if (start[1][0] == '0') {
			//cTable[3].children[2].value = start[1][1];
		} else {
			//cTable[3].children[2].value = start[1];
		}
		//cTable[3].children[2].value = start[1];
		//addOnChange(cTable[3].children[2]);
		//}
		...
		document.getElementsByClassName('btnAiCon')[0].children[0].click();
		content[0].children[1].firstChild.click();
	}
